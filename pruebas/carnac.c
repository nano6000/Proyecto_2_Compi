...
static void on_entry_activate(GtkEntry *entry, GtkWidget *widget)
{
   ...
   if (exit_message!=1)
   {
      ...
      {
         case -2:
            strcpy(message, "¡Error! La casilla ya esta ocupada seleccione otra casilla.");
            break;
         case -1:
            strcpy(message, "¡Error! La casilla indicada se encuentra fuera de los limites del tablero actual.");
            break;
         case 0:
            strcpy(message, "¡Error! Coordenadas de la casilla o colores erroneos.");
            break;
      }
